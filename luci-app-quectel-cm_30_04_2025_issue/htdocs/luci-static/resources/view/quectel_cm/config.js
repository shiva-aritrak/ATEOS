...
return view.extend({
	load: function() {
		return Promise.all([
			...
			callQtcmStatus().catch(function() { return null; })
		]);
	},

	render: function() {
		var statusData = arguments[0] && arguments[0][2];
		var detectedStatus = {};
		...
		o.placeholder = 'usb0';
		o.optional = true;
		o.description = _('Leave blank to auto-detect the modem data interface. For EC200A-CN in CDC Ethernet mode, this is usually usb0.');
		if (detectedStatus.interface)
			o.description += ' ' + _('Current detected value: %s').format(detectedStatus.interface);

		o = section.taboption('advanced', form.Value, 'at_port', _('AT port'));
		o.placeholder = _('Auto-detect');
		o.optional = true;
		o.description = _('Leave blank to auto-detect the modem control port. Set this only when detection chooses the wrong ttyUSB device.');
		if (detectedStatus.at_port)
			o.description += ' ' + _('Current detected value: %s').format(detectedStatus.at_port);
