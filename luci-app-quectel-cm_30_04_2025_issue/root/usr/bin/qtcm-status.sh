...
detect_at_port() {
	...
	if [ -f "$log_file" ]; then
		candidate="$(sed -n 's/.*Auto find qmichannel = \(.*\)$/\1/p' "$log_file" | tail -n 1)"
		if [ -n "$candidate" ]; then
			printf '%s' "$candidate"
			return 0
		fi
...
main() {
	local running=0
	local interface at_port configured_interface configured_at_port
	...
	configured_interface="$(read_uci qtcm.main.network_interface)"
	configured_at_port="$(read_uci qtcm.main.at_port)"
	[ -n "$configured_at_port" ] || configured_at_port="$(read_uci qtcm.main.at_cmd)"
	...
	printf '"configured_at_port":"%s",' "$(json_escape "$configured_at_port")"
	printf '"at_port":"%s",' "$(json_escape "$at_port")"
	printf '"configured_interface":"%s",' "$(json_escape "$configured_interface")"
